...
test("allows selecting layouts and toggling settings", async ({ app }) => {
  ...
  try {
    ...
    // Check initial RTL state and toggle to opposite
    const stateBeforeRtl = await invoke<WorkspaceState>("requestState");
    const initialRtl = stateBeforeRtl.rightToLeft;
    const expectedRtl = !initialRtl;
    ...
    // Check initial crop marks state and toggle to opposite
    const stateBeforeCrop = await invoke<WorkspaceState>("requestState");
    const initialCrop = stateBeforeCrop.showCropMarks;
    const expectedCrop = !initialCrop;
